...
class SignupViewController: UIViewController {
    ...
    @IBAction func signupButtonTapped(_ sender: UIButton) {
        ...
        if email != "" && password.count >= 6 {
            ...
            FriendSystemSitrep.system.createAccount(email, password: password, name: "Name") { (success) in
                if success {
    ...
    @IBAction func loginButtonTapped(_ sender: UIButton) {
        ...
        if email != "" && password.count >= 6 {
            FriendSystemSitrep.system.loginAccount(email, password: password) { (success) in
                if success {
                    self.performSegue(withIdentifier: "signupCompleteSegue", sender: self)
                } else {
                    // Error
